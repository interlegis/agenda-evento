...
def enviar_email_autenticar_cadastro(usuario, link):
    msg_plain = render_to_string('authentication/autenticar_cadastro.txt',
                                {'link': link})
    msg_html = render_to_string('authentication/autenticar_cadastro.html',
                                {'link': link})
    send_mail(
    u'Autenticação de Cadastro',
    msg_plain,
    'sapl-test@interlegis.leg.br',
    [usuario.email],
    fail_silently=False,
